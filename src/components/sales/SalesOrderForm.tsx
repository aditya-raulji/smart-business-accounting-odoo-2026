// Sales Order Form Component for Urban Furniture Accounting System.
// What: Interactive form for creating, viewing, and managing Sales Orders.
// Why: Provides line item management with automatic product selling price lookup, editable tax percentages per line (0%, 5%, 12%, 18%, 28%),
//      real-time Untaxed/Tax/Grand Total calculation, and seamless 1-click transition to Customer Invoice creation.
// Why not alternative: Manual order entry without line tax calculation and price auto-fill causes human errors in billing.
// Where used: /sales/orders/new and /sales/orders/[id].

"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { PageHeader } from "@/components/ui/PageHeader";
import { Card } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";
import { Input } from "@/components/ui/Input";
import { Select } from "@/components/ui/Select";
import { Badge } from "@/components/ui/Badge";
import { Plus, Trash2, ArrowLeft, CheckCircle, FileText, XCircle } from "lucide-react";
import {
  createSalesOrder,
  confirmSalesOrder,
  cancelSalesOrder,
} from "@/lib/actions/sales-orders";

interface ContactOption {
  id: string;
  name: string;
  type: string;
}

interface ProductOption {
  id: string;
  name: string;
  salesPrice: number;
}

interface AnalyticOption {
  id: string;
  name: string;
  type: string;
}

interface SOLineState {
  productId: string;
  analyticAccountId: string;
  qty: number;
  unitPrice: number;
  taxRate: number;
}

interface SalesOrderFormProps {
  initialData?: {
    id: string;
    soNumber: string;
    customerId: string;
    soDate: Date | string;
    status: "DRAFT" | "CONFIRMED" | "CANCELLED";
    lines: Array<{
      id?: string;
      productId: string;
      analyticAccountId?: string | null;
      qty: number;
      unitPrice: number;
      taxRate: number;
      lineSubtotal?: number;
      lineTax?: number;
      lineTotal?: number;
      product?: { name: string; salesPrice: number };
      analyticAccount?: { id: string; name: string };
    }>;
    createdInvoice?: { id: string; invoiceNumber: string; status: string } | null;
  };
  customers: ContactOption[];
  products: ProductOption[];
  analyticAccounts: AnalyticOption[];
}

export function SalesOrderForm({
  initialData,
  customers,
  products,
  analyticAccounts,
}: SalesOrderFormProps) {
  const router = useRouter();
  const isNew = !initialData;
  const isReadonly = !isNew && initialData.status !== "DRAFT";

  // Form State
  const [customerId, setCustomerId] = useState(
    initialData?.customerId || (customers[0]?.id ?? "")
  );
  const [soDate, setSoDate] = useState(
    initialData
      ? new Date(initialData.soDate).toISOString().split("T")[0]
      : new Date().toISOString().split("T")[0]
  );

  const [lines, setLines] = useState<SOLineState[]>(
    initialData?.lines?.length
      ? initialData.lines.map((l) => ({
          productId: l.productId,
          analyticAccountId: l.analyticAccountId || "",
          qty: Number(l.qty),
          unitPrice: Number(l.unitPrice),
          taxRate: Number(l.taxRate ?? 18),
        }))
      : [
          {
            productId: products[0]?.id ?? "",
            analyticAccountId: "",
            qty: 1,
            unitPrice: products[0]?.salesPrice ? Number(products[0].salesPrice) : 0,
            taxRate: 18,
          },
        ]
  );

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Line item handlers
  const handleProductChange = (index: number, productId: string) => {
    const selectedProd = products.find((p) => p.id === productId);
    const updated = [...lines];
    updated[index] = {
      ...updated[index],
      productId,
      unitPrice: selectedProd?.salesPrice ? Number(selectedProd.salesPrice) : updated[index].unitPrice,
    };
    setLines(updated);
  };

  const handleLineChange = (
    index: number,
    field: keyof SOLineState,
    value: any
  ) => {
    const updated = [...lines];
    updated[index] = { ...updated[index], [field]: value };
    setLines(updated);
  };

  const addLine = () => {
    const firstProd = products[0];
    setLines([
      ...lines,
      {
        productId: firstProd?.id ?? "",
        analyticAccountId: "",
        qty: 1,
        unitPrice: firstProd?.salesPrice ? Number(firstProd.salesPrice) : 0,
        taxRate: 18,
      },
    ]);
  };

  const removeLine = (index: number) => {
    if (lines.length <= 1) return;
    setLines(lines.filter((_, i) => i !== index));
  };

  // Calculations
  const lineCalculations = lines.map((l) => {
    const subtotal = (Number(l.qty) || 0) * (Number(l.unitPrice) || 0);
    const tax = subtotal * ((Number(l.taxRate) || 0) / 100);
    const total = subtotal + tax;
    return { subtotal, tax, total };
  });

  const untaxedTotal = lineCalculations.reduce((sum, item) => sum + item.subtotal, 0);
  const taxTotal = lineCalculations.reduce((sum, item) => sum + item.tax, 0);
  const grandTotal = untaxedTotal + taxTotal;

  // Actions
  const handleSaveDraft = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);

    if (!customerId) {
      setError("Please select a customer.");
      return;
    }

    if (lines.length === 0) {
      setError("At least 1 product line is required.");
      return;
    }

    for (const [i, l] of lines.entries()) {
      if (!l.productId) {
        setError(`Line ${i + 1}: Please select a product.`);
        return;
      }
      if (l.qty <= 0) {
        setError(`Line ${i + 1}: Quantity must be greater than 0.`);
        return;
      }
      if (l.unitPrice < 0) {
        setError(`Line ${i + 1}: Unit price cannot be negative.`);
        return;
      }
    }

    setLoading(true);

    const res = await createSalesOrder({
      customerId,
      soDate,
      lines: lines.map((l) => ({
        productId: l.productId,
        analyticAccountId: l.analyticAccountId || null,
        qty: Number(l.qty),
        unitPrice: Number(l.unitPrice),
        taxRate: Number(l.taxRate),
      })),
    });

    setLoading(false);

    if (res.error) {
      setError(res.error);
    } else if (res.id) {
      router.push(`/sales/orders/${res.id}`);
      router.refresh();
    }
  };

  const handleConfirm = async () => {
    if (!initialData) return;
    setError(null);
    setLoading(true);

    const res = await confirmSalesOrder(initialData.id);
    setLoading(false);

    if (res.error) {
      setError(res.error);
    } else {
      router.refresh();
    }
  };

  const handleCancel = async () => {
    if (!initialData) return;
    if (!confirm("Are you sure you want to cancel this Sales Order?")) return;

    setError(null);
    setLoading(true);

    const res = await cancelSalesOrder(initialData.id);
    setLoading(false);

    if (res.error) {
      setError(res.error);
    } else {
      router.refresh();
    }
  };

  return (
    <div className="space-y-6">
      <PageHeader
        title={
          isNew
            ? "New Sales Order"
            : `Sales Order: ${initialData.soNumber}`
        }
        subtitle={
          isNew
            ? "Draft customer sales quotation or order with itemized taxes."
            : `Order Date: ${new Date(initialData.soDate).toLocaleDateString("en-IN")}`
        }
        action={
          <div className="flex items-center gap-2">
            {!isNew && initialData.status === "CONFIRMED" && (
              initialData.createdInvoice ? (
                <Link href={`/sales/invoices/${initialData.createdInvoice.id}`}>
                  <Button variant="primary" size="sm">
                    <FileText size={14} className="mr-1" />
                    View Invoice ({initialData.createdInvoice.invoiceNumber})
                  </Button>
                </Link>
              ) : (
                <Link href={`/sales/invoices/new?fromSO=${initialData.id}`}>
                  <Button variant="primary" size="sm">
                    <FileText size={14} className="mr-1" />
                    Create Customer Invoice
                  </Button>
                </Link>
              )
            )}
          </div>
        }
      />

      {error && (
        <div className="p-4 bg-red-50 border border-red-200 text-red-700 rounded-sm text-sm">
          {error}
        </div>
      )}

      {/* Linked Invoice Notice */}
      {!isNew && initialData.createdInvoice && (
        <div className="bg-[#FFFDF8] border border-[#E2D9CC] p-4 rounded-sm flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-[#171717]">
            <CheckCircle size={16} className="text-green-600" />
            <span>
              Customer Invoice <strong>{initialData.createdInvoice.invoiceNumber}</strong> has been generated for this Sales Order.
            </span>
          </div>
          <Link
            href={`/sales/invoices/${initialData.createdInvoice.id}`}
            className="text-xs font-semibold text-[#B91C1C] hover:underline"
          >
            View Customer Invoice →
          </Link>
        </div>
      )}

      <form onSubmit={handleSaveDraft} className="space-y-6">
        <Card>
          {/* Header Metadata */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pb-6 border-b border-[#E2D9CC]">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#3D3A36] mb-1">
                Customer *
              </label>
              {isReadonly ? (
                <div className="text-sm font-semibold text-[#171717]">
                  {customers.find((c) => c.id === customerId)?.name || "Customer"}
                </div>
              ) : (
                <Select
                  value={customerId}
                  onChange={(e) => setCustomerId(e.target.value)}
                  required
                >
                  {customers.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.name} ({c.type})
                    </option>
                  ))}
                </Select>
              )}
            </div>

            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#3D3A36] mb-1">
                Order Date *
              </label>
              {isReadonly ? (
                <div className="text-sm font-semibold text-[#171717]">
                  {new Date(soDate).toLocaleDateString("en-IN")}
                </div>
              ) : (
                <Input
                  type="date"
                  value={soDate}
                  onChange={(e) => setSoDate(e.target.value)}
                  required
                />
              )}
            </div>

            {!isNew && (
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-[#3D3A36] mb-1">
                  Status
                </label>
                <div className="pt-1.5">
                  <Badge
                    variant={
                      initialData.status === "CONFIRMED"
                        ? "success"
                        : initialData.status === "DRAFT"
                        ? "warning"
                        : "danger"
                    }
                  >
                    {initialData.status}
                  </Badge>
                </div>
              </div>
            )}
          </div>

          {/* Line Items Table */}
          <div className="space-y-4 pt-4">
            <h3 className="text-sm font-bold text-[#171717]">Order Product Lines</h3>

            <div className="overflow-x-auto border border-[#E2D9CC] rounded-sm">
              <table className="w-full text-left text-xs">
                <thead className="bg-[#FFFDF8] border-b border-[#E2D9CC] text-[#3D3A36] font-semibold uppercase tracking-wider">
                  <tr>
                    <th className="px-3 py-2 w-12 text-center">Sr.</th>
                    <th className="px-3 py-2">Product</th>
                    <th className="px-3 py-2">Budget Analytics</th>
                    <th className="px-3 py-2 w-24 text-right">Qty</th>
                    <th className="px-3 py-2 w-28 text-right">Unit Price (₹)</th>
                    <th className="px-3 py-2 w-24 text-right">Tax %</th>
                    <th className="px-3 py-2 w-24 text-right">Tax (₹)</th>
                    <th className="px-3 py-2 w-28 text-right">Line Total (₹)</th>
                    {!isReadonly && <th className="px-3 py-2 w-12 text-center"></th>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#E2D9CC]">
                  {lines.map((line, idx) => (
                    <tr key={idx} className="hover:bg-[#FFFDF8]">
                      <td className="px-3 py-2 text-center text-[#3D3A36]">{idx + 1}</td>
                      <td className="px-3 py-2">
                        {isReadonly ? (
                          <span className="font-medium text-[#171717]">
                            {products.find((p) => p.id === line.productId)?.name || "—"}
                          </span>
                        ) : (
                          <Select
                            value={line.productId}
                            onChange={(e) => handleProductChange(idx, e.target.value)}
                            required
                          >
                            {products.map((p) => (
                              <option key={p.id} value={p.id}>
                                {p.name}
                              </option>
                            ))}
                          </Select>
                        )}
                      </td>

                      <td className="px-3 py-2">
                        {isReadonly ? (
                          <span className="text-[#3D3A36]">
                            {analyticAccounts.find((a) => a.id === line.analyticAccountId)?.name || "—"}
                          </span>
                        ) : (
                          <Select
                            value={line.analyticAccountId}
                            onChange={(e) => handleLineChange(idx, "analyticAccountId", e.target.value)}
                          >
                            <option value="">None</option>
                            {analyticAccounts.map((a) => (
                              <option key={a.id} value={a.id}>
                                {a.name} ({a.type})
                              </option>
                            ))}
                          </Select>
                        )}
                      </td>

                      <td className="px-3 py-2 text-right">
                        {isReadonly ? (
                          <span>{line.qty}</span>
                        ) : (
                          <Input
                            type="number"
                            min="1"
                            step="1"
                            value={line.qty}
                            onChange={(e) => handleLineChange(idx, "qty", parseFloat(e.target.value) || 0)}
                            className="text-right"
                            required
                          />
                        )}
                      </td>

                      <td className="px-3 py-2 text-right">
                        {isReadonly ? (
                          <span>₹{line.unitPrice.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
                        ) : (
                          <Input
                            type="number"
                            min="0"
                            step="0.01"
                            value={line.unitPrice}
                            onChange={(e) => handleLineChange(idx, "unitPrice", parseFloat(e.target.value) || 0)}
                            className="text-right"
                            required
                          />
                        )}
                      </td>

                      <td className="px-3 py-2 text-right">
                        {isReadonly ? (
                          <span>{line.taxRate}%</span>
                        ) : (
                          <Select
                            value={line.taxRate}
                            onChange={(e) => handleLineChange(idx, "taxRate", parseFloat(e.target.value) || 0)}
                          >
                            <option value={0}>0%</option>
                            <option value={5}>5%</option>
                            <option value={12}>12%</option>
                            <option value={18}>18%</option>
                            <option value={28}>28%</option>
                          </Select>
                        )}
                      </td>

                      <td className="px-3 py-2 text-right text-[#78716C]">
                        ₹{lineCalculations[idx].tax.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>

                      <td className="px-3 py-2 text-right font-semibold text-[#171717]">
                        ₹{lineCalculations[idx].total.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>

                      {!isReadonly && (
                        <td className="px-3 py-2 text-center">
                          {lines.length > 1 && (
                            <button
                              type="button"
                              onClick={() => removeLine(idx)}
                              className="text-red-500 hover:text-red-700 p-1"
                              title="Remove Line"
                            >
                              <Trash2 size={14} />
                            </button>
                          )}
                        </td>
                      )}
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>

            {!isReadonly && (
              <Button
                type="button"
                variant="outline"
                size="sm"
                onClick={addLine}
                className="mt-2"
              >
                <Plus size={14} className="mr-1" />
                Add Product Line
              </Button>
            )}

            {/* Total Footer */}
            <div className="flex justify-end pt-4 border-t border-[#E2D9CC]">
              <div className="w-72 bg-[#FFFDF8] border border-[#E2D9CC] p-4 rounded-sm space-y-2 text-xs">
                <div className="flex justify-between text-[#3D3A36]">
                  <span>Untaxed Amount:</span>
                  <span>
                    ₹{untaxedTotal.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
                <div className="flex justify-between text-[#78716C]">
                  <span>Tax Amount:</span>
                  <span>
                    ₹{taxTotal.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
                <div className="flex justify-between font-bold text-sm text-[#171717] pt-2 border-t border-[#E2D9CC]">
                  <span>Grand Total:</span>
                  <span className="text-[#B91C1C]">
                    ₹{grandTotal.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            </div>
          </div>
        </Card>

        {/* Action Buttons Toolbar */}
        <div className="flex items-center justify-between bg-white border border-[#E2D9CC] p-4 rounded-sm">
          <Link
            href="/sales/orders"
            className="inline-flex items-center gap-1 px-4 py-2 rounded-sm text-xs font-semibold bg-white border border-[#E2D9CC] text-[#171717] hover:bg-[#F7F4EE]"
          >
            <ArrowLeft size={14} />
            Back to Sales Orders
          </Link>

          <div className="flex items-center gap-3">
            {isNew && (
              <Button type="submit" variant="primary" disabled={loading}>
                {loading ? "Saving..." : "Save Draft SO"}
              </Button>
            )}

            {!isNew && initialData.status === "DRAFT" && (
              <>
                <Button
                  type="button"
                  variant="outline"
                  onClick={handleCancel}
                  disabled={loading}
                  className="border-red-300 text-red-600 hover:bg-red-50"
                >
                  <XCircle size={14} className="mr-1" />
                  Cancel
                </Button>

                <Button
                  type="button"
                  variant="primary"
                  onClick={handleConfirm}
                  disabled={loading}
                >
                  <CheckCircle size={14} className="mr-1" />
                  {loading ? "Confirming..." : "Confirm SO"}
                </Button>
              </>
            )}

            {!isNew && initialData.status === "CONFIRMED" && !initialData.createdInvoice && (
              <>
                <Button
                  type="button"
                  variant="outline"
                  onClick={handleCancel}
                  disabled={loading}
                  className="border-red-300 text-red-600 hover:bg-red-50"
                >
                  <XCircle size={14} className="mr-1" />
                  Cancel Order
                </Button>

                <Link href={`/sales/invoices/new?fromSO=${initialData.id}`}>
                  <Button type="button" variant="primary">
                    <FileText size={14} className="mr-1" />
                    Create Customer Invoice
                  </Button>
                </Link>
              </>
            )}
          </div>
        </div>
      </form>
    </div>
  );
}
